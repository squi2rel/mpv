...
typedef struct mpv_audio_output_cb_info {
    ...
    int sample_rate;
    int channels;            // always 2
    int samples;             // stereo frames in this callback
    uint64_t bytes;          // samples * channels * 2
    uint64_t sequence;       // monotonically increasing chunk id
    uint64_t dropped_samples; // cumulative dropped stereo frames
 ...
 * endian stereo PCM. Callback data is valid only until the callback returns.
 *
